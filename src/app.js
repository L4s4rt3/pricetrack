import { fetchAllRecords, addRecord as dbAddRecord, addRecords as dbAddRecords, deleteRecord as dbDeleteRecord, deleteAllRecords as dbDeleteAllRecords, subscribeToChanges, normalizeRow } from './database.js'
...
async function deleteAllRecords() {
  ...
  if (!confirm(`Confirmación: vas a eliminar ${data.length} registros. ¿Seguro?`)) return
  try {
...
const COLUMN_MAPPINGS = {
  product: ['producto', 'product', 'articulo', 'artículo', 'referencia', 'sku', 'codigo', 'nombre', 'name', 'item', 'article', 'product name'],
  category: ['categoria', 'categoría', 'category', 'departamento', 'sección', 'seccion', 'familia', 'grupo', 'group', 'tipo', 'type', 'clase'],
  price: ['precio', 'price', 'pvp', 'coste', 'importe', 'baseiva', 'base iva', 'valor', 'value', 'amount', 'importe_total', 'total', 'cost'],
  unit: ['unidad', 'unit', 'unid', 'medida', 'um', 'kg', 'kilos', 'litros', 'medida', 'u.m.', 'unidades'],
  year: ['año', 'ano', 'year', 'ejercicio', 'année', 'anio', 'periodo'],
  month: ['mes', 'month', 'mois'],
  notes: ['notas', 'notes', 'observaciones', 'comentario', 'comentarios', 'obs', 'descripcion', 'description'],
  date: ['fecha', 'date', 'fecha fra.', 'fechafra', 'fecha_fra', 'fecha_factura', 'fecha documento']
}
...
function extractMonth(val) {
  ...
  const dateParts = s.match(/^\d{1,2}\/(\d{1,2})/)
  if (dateParts) return parseInt(dateParts[1])
  const d = new Date(s)
  return isNaN(d.getTime()) ? null : d.getMonth() + 1
}

function normalizarColumnas(datos) {
  if (!datos || !datos.length) return []

  const keys = Object.keys(datos[0])
  const map = {}

  for (const [campo, sinonimos] of Object.entries(COLUMN_MAPPINGS)) {
    const keyLower = (k) => k.toLowerCase().trim().replace(/[_\s-]+/g, ' ')
  ...
  return datos.map(fila => {
    ...
    const getVal = (campo) => map[campo] ? String(fila[map[campo]] || '') : ''

    producto = getVal('product') || ''
    categoria = getVal('category') || 'Sin categoría'
    precio = map.price ? parseNumber(fila[map.price]) : NaN
    unidad = getVal('unit') || '€/ud'
    notas = getVal('notes') || ''
    ...
    if (rawDate && (!rawYear || !rawMonth)) {
      año = extractYear(rawDate)
      mes = extractMonth(rawDate)
    }
    if (rawYear && isNaN(año)) año = extractYear(rawYear)
    if (rawMonth && mes === null) mes = extractMonth(rawMonth)

    if (isNaN(año)) año = new Date().getFullYear()

    return { product: producto, category: categoria, price: precio, unit: unidad, year: año, month: mes, notes: notas }
  }).filter(r => r.product && !isNaN(r.price) && !isNaN(r.year))
}
...
async function handleFileSelect(e) {
  ...
  try {
    if (['xlsx', 'xls'].includes(ext) && typeof XLSX !== 'undefined') {
      ...
      datosLimpios = XLSX.utils.sheet_to_json(ws, { defval: '' })
      ...
      if (result.errors?.length) {
        console.warn('Advertencias PapaParse:', result.errors.filter(e => e.type !== 'FieldMismatch'))
      }
      datosLimpios = result.data
    } else if (ext === 'csv') {
      const buf = await file.arrayBuffer()
  ...
  if (!datosLimpios.length) { showToast('⚠ El archivo no tiene datos'); return }
  console.log('Datos crudos leídos:', datosLimpios.slice(0, 3))

  importData = datosLimpios
  mostrarPreview(datosLimpios)

  const mapeados = normalizarColumnas(datosLimpios)
  if (mapeados.length) {
    importData = mapeados
    await ejecutarImportacion()
  } else {
    showToast('⚠ No se pudieron mapear las columnas')
  }
}
...
function mostrarPreview(datos) {
  if (!datos || !datos.length) return
  const keys = Object.keys(datos[0])
  const maxRows = Math.min(5, datos.length)
  const thead = `<thead><tr>${keys.map(h => `<th>${h}</th>`).join('')}</tr></thead>`
  const tbody = `<tbody>${datos.slice(0, maxRows).map(r =>
    `<tr>${keys.map(k => `<td>${r[k] ?? ''}</td>`).join('')}</tr>`
  ).join('')}</tbody>`
  document.getElementById('preview-table').innerHTML = thead + tbody
  document.getElementById('preview-info').textContent = `${datos.length} filas · ${keys.length} columnas`
...
async function importCSV() {
  ...
  if (!importData[0].product) {
    importData = normalizarColumnas(importData)
  }
