CREATE EXTENSION IF NOT EXISTS pg_trgm WITH SCHEMA extensions;

DO $$
BEGIN
  IF to_regclass('public.precios') IS NOT NULL THEN
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_precios_cliente_trgm ON public.precios USING gin (cliente gin_trgm_ops)';
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_precios_denominacion_social_trgm ON public.precios USING gin (denominacion_social gin_trgm_ops)';
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_precios_documento_trgm ON public.precios USING gin (documento gin_trgm_ops)';
    EXECUTE 'CREATE INDEX IF NOT EXISTS idx_precios_factura_trgm ON public.precios USING gin (factura gin_trgm_ops)';

    EXECUTE $view$
      CREATE OR REPLACE VIEW public.precios_dashboard_mensual
      WITH (security_invoker = on) AS
      WITH monthly_keys AS (
        SELECT DISTINCT
          make_date(ano, COALESCE(mes, 1), 1) AS month_start,
          ano,
          mes
        FROM public.precios
        WHERE ano IS NOT NULL
          AND (mes IS NULL OR mes BETWEEN 1 AND 12)
        ORDER BY month_start DESC
        LIMIT 6
      )
      SELECT
        monthly_keys.month_start,
        monthly_keys.ano,
        monthly_keys.mes,
        count(*)::integer AS lineas,
        count(DISTINCT COALESCE(NULLIF(denominacion_social, ''), NULLIF(cliente, '')))::integer AS clientes,
        count(DISTINCT NULLIF(producto, ''))::integer AS productos,
        COALESCE(sum(kilos), 0)::numeric(14, 2) AS kilos,
        COALESCE(sum(base_iva), 0)::numeric(14, 2) AS facturacion,
        CASE
          WHEN COALESCE(sum(kilos), 0) > 0 THEN (COALESCE(sum(base_iva), 0) / NULLIF(sum(kilos), 0))::numeric(12, 4)
          ELSE avg(precio)::numeric(12, 4)
        END AS precio_medio,
        max(created_at) AS refreshed_at
      FROM public.precios
      JOIN monthly_keys
        ON public.precios.ano = monthly_keys.ano
        AND public.precios.mes IS NOT DISTINCT FROM monthly_keys.mes
      GROUP BY monthly_keys.month_start, monthly_keys.ano, monthly_keys.mes
    $view$;
  ELSE
    EXECUTE $view$
      CREATE OR REPLACE VIEW public.precios_dashboard_mensual
      WITH (security_invoker = on) AS
      SELECT
        NULL::date AS month_start,
        NULL::integer AS ano,
        NULL::integer AS mes,
        0::integer AS lineas,
        0::integer AS clientes,
        0::integer AS productos,
        0::numeric(14, 2) AS kilos,
        0::numeric(14, 2) AS facturacion,
        0::numeric(12, 4) AS precio_medio,
        NULL::timestamptz AS refreshed_at
      WHERE false
    $view$;
  END IF;
END $$;

GRANT SELECT ON public.precios_dashboard_mensual TO anon, authenticated;
